...
| 項目 | 内容 |
|------|------|
| ファイル | `E:\hiro\Claude\TODO\index.html`（約3500行、1ファイル完結） |
...
- TaskモデルのprojectIdとの紐づけはフェーズ2で追加。
...
const S = {
  ...
  projects: [],           // Project[]
  filter: { status: 'all', category: 'all', date: 'all' },
...
| `saveTasks()` / `saveGoals()` / `saveBlocks()` / `saveThemes()` / `saveCompletions()` / `saveBlockTemplates()` / `saveProjects()` | 各エンティティを localStorage に書き込む（アロー関数定数） |
...
| `deleteProject(id)` | Project を削除（フェーズ2でTask.projectIdも連動） |
| `getProjectById(id)` | ID で Project を取得 |
| `getProjectTasks(projectId)` | プロジェクトに紐づく親タスク一覧（フェーズ2で有効化） |
...
| `buildDecomposePrompt(goal)` | `goal.type` に応じてAI分解プロンプトを切り替えて返す |
...
| `addSelectedAiTasks(tasks, goalId)` | 選択タスクを Goal のカテゴリ/テーマ・recurrence付きで追加 |
...
| `gistPush()` | 全データを GitHub Gist に保存（初回 POST、以降 PATCH） |
| `gistPull()` | GitHub Gist からデータを読み込み上書き |
...
- **AI分解**: `buildDecomposePrompt(goal)` で type に応じたプロンプトを生成。
  ...
  - `habit`: 12ヶ月分の繰り返しタスク（`recurrence` + `recurrenceEnd` 付き）
...
| API | 用途 | キー |
|-----|------|------|
| Gemini API | AIタスク分解・Goal提案 | `todo_gemini_api_key` |
