...
```json
{
  ...
  "category": "仕事 | 個人 | その他",
  "theme": "テーマ名",
  "type": "achievement | habit | explore | dormant | null",
...
| `decomposeGoal(goalId)` | 目標からタスクを提案。dormant ゴールはトーストを表示して早期リターン。内部で `buildDecomposePrompt()` を呼ぶ |
...
| `buildSuggestPrompt(goals)` | 未着手/進行中ゴールを2セクションに分けてGeminiへのプロンプトを構築。`done` と `dormant` は除外 |
...
| `onGoalTypeChange(prefix)` | typeラジオ変更時に「状況メモ」欄の表示/非表示を切り替え。dormant 選択時はステータスを notStarted に固定して `<select>` を disabled にする |
...
**habit型Goalにはタスクを紐づけない。**
習慣の実行は振り返り（review）の○/×で追跡する。タスクを紐づけると振り返りに重複表示される。

| タスクの種類 | goalId | recurrence | blockIds |
|-------------|--------|-----------|---------|
| 日常ルーティン（バッファ時間確保など） | null | あり | あり/なし |
| achievement/explore Goal のタスク | そのGoalのid | **なし** | なし |
| habit Goal のタスク | **紐づけ禁止** | **禁止** | 禁止 |
| 通常の期日タスク | null or achievement Goal | なし | なし |

- `goalId=null` + `recurrence` あり → 日常ルーティン（blockとの紐づけ可）
- habit型GoalのgoalIdを持つタスクは `openReviewModal()` の taskItems から除外される（コード上の担保）

### 7. 完了判定の優先順位（`isTaskDone`）
...
### 9. Goal の type システム
Goal には `type`（達成型 / 習慣型 / 探索型 / 休止型）を任意で設定できる。

- **UI**: 追加・編集モーダルのラジオボタンで選択。探索型のみ「状況メモ（`progressNote`）」テキストエリアを表示。
- **グリッドバッジ**: 各セルに種類バッジを表示（達成=青 / 習慣=緑 / 探索=橙 / 休止=グレー）。`type: null` は非表示。休止型セルは `opacity:0.6` で半透明表示。
- **AI分解**: `buildDecomposePrompt(goal)` で type に応じたプロンプトを生成。
  ...
  - `dormant`: `decomposeGoal()` でトーストを表示して早期リターン（AI呼び出しなし）
  - `null`: 汎用プロンプト
- **今月提案**: `suggestGoals()` → `buildSuggestPrompt()` で未着手を優先・進行中を補足として2セクションに分けてGeminiに送信。`dormant` は除外。結果モーダルにステータス・種類バッジを表示し、「🤖 AI分解する」で即座にAI分解へ遷移。
- **グループビュー**: 通常ゴールはカテゴリ→テーマでグループ化。`dormant` ゴールは末尾に「💤 休止中」セクションとして独立表示（`opacity:0.6`）。
- **ステータス制約**: `dormant` 選択時はステータスを `notStarted` に固定。`gd-status` / `ga-status` の `<select>` を `disabled` にする。既存データの後方互換を保つ（古いGoalは `type: null` のまま）。
